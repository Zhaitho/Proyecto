...
public class Parser {
    ...
    public Parser(Lexer lexer) {
        this.lexer = lexer;
        this.currentToken = lexer.nextToken(); // Primer token
    }
    ...
    private double factor() {
        ...
        if (currentToken.getTipoToken().equals("NUMBER")) {
            ...
            consume("RPAREN");
            if (argumento <= 0) {
                throw new ArithmeticException("Logaritmo de un número no positivo");
            }
            result = Math.log(argumento);
        } else if (currentToken.getTipoToken().equals("SIN")) {
            consume("SIN");
            consume("LPAREN");
            double argumento = expression();
            consume("RPAREN");
            result = Math.sin(argumento);
        } else if (currentToken.getTipoToken().equals("COS")) {
            consume("COS");
            consume("LPAREN");
            double argumento = expression();
            consume("RPAREN");
            result = Math.cos(argumento);
        } else {
